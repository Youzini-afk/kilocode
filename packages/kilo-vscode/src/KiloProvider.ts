...
import { fetchAndSendIndexingStatus as fetchIndexingStatus } from "./kilo-provider/indexing-status"
import { fetchPlugins, routePluginWebviewMessage, type PluginRouteContext } from "./kilo-provider/plugin-messages"
...
export class KiloProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
  ...
  private pluginCtx(): PluginRouteContext {
    return {
      state: this.connectionState,
      cached: () => this.cachedPluginsMessage,
      setCached: (msg) => {
        this.cachedPluginsMessage = msg
      },
      server: () => this.connectionService.getServerConfig(),
      workspace: () => this.getWorkspaceDirectory(this.currentSession?.id),
      project: () => this.getProjectDirectory(this.currentSession?.id),
      post: (msg) => this.postMessage(msg),
      refresh: () => this.refreshAfterPluginChange(),
      config: () => this.fetchAndSendConfigUpdated(),
      openFile: (file) => this.handleOpenFile(file),
  ...
  private async routePrehandledWebviewMessage(message: any): Promise<boolean> {
    await routeSuggestionWebviewMessage(this.questionCtx, message)
    if (await ModelState.handleMessage(message.type, message, this.client, (msg) => this.postMessage(msg))) return true
    if (await routeAutocompleteMessage(message, (msg) => this.postMessage(msg))) return true
    if (await routePluginWebviewMessage(message, this.pluginCtx())) return true
    return handleSidebarWorktreeMessage(message, {
      post: (msg) => this.postMessage(msg),
  ...
  private setupWebviewMessageHandler(webview: vscode.Webview): void {
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      if (await this.routePrehandledWebviewMessage(message)) return
      switch (message.type) {
        case "webviewReady":
  ...
  private async fetchAndSendPlugins(): Promise<void> {
    await fetchPlugins(this.pluginCtx())
  }
  ...
  private async fetchAndSendConfig(): Promise<void> {
    await fetchConfig({
      ...
      cached: () => this.cachedConfigMessage,
      setCached: (msg) => {
        this.cachedConfigMessage = msg
      },
      workspace: () => this.getWorkspaceDirectory(),
      global: async () => (await this.client!.global.config.get({ throwOnError: true })).data,
      setGlobalCached: (config) => {
  ...
  private async fetchAndSendIndexingStatus(): Promise<void> {
    await fetchIndexingStatus({
      cached: () => this.cachedIndexingStatusMessage,
      setCached: (msg) => {
  ...
  private async handleUpdateConfig(partial: Partial<Config>, project: Partial<Config> = {}): Promise<void> {
    ...
    try {
      ...
      const features = configFeatures(merged)
      ...
      const globalConfig: Config | undefined = hasGlobal
        ? mergeConfigPatches(this.cachedGlobalConfig, partial)
        : (this.cachedGlobalConfig ?? undefined)
      const features = configFeatures(optimistic)
      if (hasGlobal) this.cachedGlobalConfig = globalConfig ?? null
      this.cachedConfigMessage = {
        type: "configLoaded",
        config: optimistic,
