...
interface KiloConfig {
    ...
    // Kilo allows plugins as plain strings or [name, options] tuples.
 ...
 * Checks: Kilo compaction, DCP plugin, OMO conflicting hooks.
 */
export function detectConflicts(directory: string): ConflictResult {
    ...
    // --- Check Kilo compaction config ---
    const compactionResult = checkCompaction(directory);
    if (compactionResult.auto) {
        conflicts.compactionAuto = true;
        reasons.push("Kilo auto-compaction is enabled (compaction.auto=true/default)");
    }
    if (compactionResult.prune) {
        conflicts.compactionPrune = true;
        reasons.push("Kilo prune is enabled (compaction.prune=true)");
    }
...
function checkCompaction(directory: string): { auto: boolean; prune: boolean } {
    ...
    // Built-in Kilo Magic Context owns context management by default. Only an
    // explicit user/project compaction setting should be treated as a conflict.
    return { auto: false, prune: false };
}
...
function readProjectCompaction(directory: string): {
    ...
    // .kilo/.kilocode config has higher precedence
    const dotOcConfig =
        readJsoncFile<KiloConfig>(join(directory, ".kilo", "kilo.jsonc")) ??
        readJsoncFile<KiloConfig>(join(directory, ".kilo", "kilo.json")) ??
...
function collectPluginEntries(directory: string): string[] {
    ...
    for (const configPath of [
        ...
        const config = readJsoncFile<KiloConfig>(configPath);
