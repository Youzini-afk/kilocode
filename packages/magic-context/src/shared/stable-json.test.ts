...
describe("stableStringify", () => {
    ...
    test("shared non-circular references render normally", () => {
        const child = { z: 1 };
        const input = { b: child, a: child };
        expect(stableStringify(input)).toBe('{"a":{"z":1},"b":{"z":1}}');
    });

    test("calls do not share circular reference state", () => {
        const input = { z: 1 };
        expect(stableStringify(input)).toBe('{"z":1}');
        expect(stableStringify(input)).toBe('{"z":1}');
    });
