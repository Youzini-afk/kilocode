...
function getUserLegacyConfigBasePath(): string {
    return join(process.env.KILO_CONFIG_DIR?.trim() || getKiloConfigDir(), LEGACY_CONFIG_FILE_BASENAME);
}

function getProjectConfigBasePaths(directory: string): string[] {
    return [
        join(directory, ".kilo", CONFIG_FILE_BASENAME),
        join(directory, ".kilocode", CONFIG_FILE_BASENAME),
        join(directory, CONFIG_FILE_BASENAME),
        join(directory, ".kilo", LEGACY_CONFIG_FILE_BASENAME),
        join(directory, ".kilocode", LEGACY_CONFIG_FILE_BASENAME),
        join(directory, LEGACY_CONFIG_FILE_BASENAME),
    ];
}
...
function parsePluginConfig(
    ...
    for (const key of errorPaths) {
        ...
        if (isAgentConfig) {
            ...
            delete patched[key];
            warnings.push(
                `"${key}": invalid agent configuration, ignoring. Check your kilo-magic-context.jsonc.`,
            );
        } else {
