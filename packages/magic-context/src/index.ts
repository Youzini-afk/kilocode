...
import { setModelContextLimitOverrides } from "./hooks/magic-context/event-resolvers";
import { type ConflictResult, detectConflicts } from "./shared/conflict-detector";
import { getMagicContextStorageDir, setKiloRuntimeInfo } from "./shared/data-path";
...
const server: Plugin = async (ctx) => {
    ...
    if (pluginConfig.configWarnings?.length) {
        ...
        const warningText = [
            ...
            "",
            "Check your `kilo-magic-context.jsonc` to fix these values.",
        ].join("\n");
    ...
    if (pluginConfig.enabled) {
        ...
        // Warm the model-context-limit cache from Kilo's SDK and refresh
        // periodically. The API response matches Kilo's internal resolution
        // (live models.dev cache + compiled-in snapshot + custom provider overrides
        // + derived experimental modes), so any model Kilo knows the limit
    ...
    // runs right after `chat.message` in Kilo's prompt flow, so this
    ...
    return {
        ...
        "chat.message": async (input, _output) => {
            ...
            // Kilo's prompt flow (see session/prompt.ts), so by the time
