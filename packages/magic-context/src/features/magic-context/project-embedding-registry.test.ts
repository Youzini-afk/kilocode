...
import { saveEmbedding } from "./memory/storage-memory-embeddings";
...
describe("project embedding registry", () => {
    ...
    it("uses observation fingerprints and disables runtime reads for corrupt first-time config", () => {
    ...
    it("wipes stale memory embeddings when provider identity changes", () => {
        const db = useTempDb();
        ...
        saveEmbedding(db, memory.id, new Float32Array([1, 2]), "old-model");
    ...
    it("does not wipe existing embeddings in observation mode", () => {
        const db = useTempDb();
        const memory = insertMemory(db, {
            projectPath: "git:project",
            category: "USER_DIRECTIVES",
        ...
        registerProjectInObservationMode(
            ...
            "git:project",
            "/tmp/project",
            localConfig("fallback-model"),
            "embedding config recovery",
        );
