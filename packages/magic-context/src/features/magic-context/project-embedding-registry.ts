...
function getRuntimeFingerprint(config: EmbeddingConfig): string {
    ...
    const fingerprintConfig =
        config.provider === "openai-compatible"
            ...
            : config;
    return `${getEmbeddingProviderIdentity(config)}:${sha256Prefix(stableStringify(fingerprintConfig))}`;
}
...
function anyStoredModelIdIsStale(ids: Set<string | null>, current: string): boolean {
...
export async function sweepAllRegisteredProjects(
    ...
    const deadline = Date.now() + SWEEP_MAX_WALL_CLOCK_MS;
