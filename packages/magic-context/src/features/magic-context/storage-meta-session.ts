...
export function getOrCreateSessionMeta(db: Database, sessionId: string): SessionMeta {
    ...
    //
    // Harness gate: this fallback opens Kilo's kilo.db read-only to
    // probe `session.parent_id`. Pi has no kilo.db and no concept of
    // Kilo-style subagents — calling the fallback there throws "unable
    ...
    const fallbackSubagent =
        getHarness() === "kilo" || getHarness() === "opencode"
            ? resolveIsSubagentFromOpenCodeDb(sessionId)
            : null;
