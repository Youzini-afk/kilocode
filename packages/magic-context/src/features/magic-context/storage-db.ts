...
export function initializeDatabase(db: Database): void {
    ...
    CREATE TABLE IF NOT EXISTS tags (
      ...
      byte_size INTEGER,
      tag_number INTEGER,
      harness TEXT NOT NULL DEFAULT 'kilo',
...
CREATE INDEX IF NOT EXISTS idx_dream_queue_pending ON dream_queue(started_at, enqueued_at);
  ...
  `);
    ...
    // `harness` captures origin for dashboards and future import tooling.
    // Fresh Kilo databases default to `kilo`; explicit legacy imports keep
    // their source labels when the migration CLI copies older rows.
    ensureColumn(db, "tags", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "pending_ops", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "source_contents", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "compartments", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "compression_depth", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "session_facts", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "session_meta", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "recomp_compartments", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "recomp_facts", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
    ensureColumn(db, "message_history_index", "harness", "TEXT NOT NULL DEFAULT 'kilo'");
