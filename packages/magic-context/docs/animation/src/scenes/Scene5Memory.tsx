...
import { COLORS, FONT_FAMILY, PANEL_W, PANEL_H, SCENE_5_DURATION, MEMORY_ITEMS, INTRO_DURATION } from "../constants"
...
export const Scene5Memory: React.FC = () => {
  ...
  const closeProgress = interpolate(frame, [SESSION_CLOSE_START, SESSION_CLOSE_END], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })

  // Session open animation
  const openProgress = interpolate(frame, [SESSION_OPEN_START, SESSION_OPEN_END], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })
  ...
  // "New session" label
  const newSessionOpacity = interpolate(frame, [SESSION_OPEN_END - 10, SESSION_OPEN_END], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })
    ...
    <AbsoluteFill style={{ background: COLORS.bg }}>
      ...
      >
        ...
        <div style={{ padding: "18px 20px", overflow: "hidden" }}>
          {/* Memory block */}
          {frame >= MEMORY_ENTER && <MemoryBlock items={MEMORY_ITEMS} enterFrame={MEMORY_ENTER} />}

          {/* First message referencing memory */}
          {frame >= MESSAGE_ENTER && (
            <div style={{ marginTop: 20 }}>
              <SkeletonMessage widthPercent={85} role="assistant" enterFrame={MESSAGE_ENTER} tag={1} />
              <div
                style={{
                  marginTop: 8,
                  marginLeft: 20,
