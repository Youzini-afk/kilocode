...
import { COLORS, FONT_FAMILY, PANEL_W, PANEL_H, SCENE_2_DURATION, SCENE2_MESSAGES, INTRO_DURATION } from "../constants"
...
export const Scene2Reduce: React.FC = () => {
  ...
  const dropP = (i: number) => {
    if (i !== 5 && i !== 7) return 0
    const delay = i === 5 ? 0 : 8
    return interpolate(frame, [DROP_START + delay, DROP_END + delay], [0, 1], {
  ...
  const contextPct = interpolate(frame, [DROP_START, DROP_END + 20], [62, 34], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })
  ...
  const sceneEndFade = interpolate(globalFrame, [SCENE_2_DURATION - 15, SCENE_2_DURATION], [1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })
  ...
  return (
    <AbsoluteFill style={{ background: COLORS.bg, opacity: sceneEndFade }}>
      ...
      <SceneCaption text="Agent chooses what to drop" frame={globalFrame} />
      ...
      {frame >= NUDGE_ENTER && frame < NUDGE_EXIT && (
        <div style={{ opacity: uiOpacity }}>
          <NudgeBanner level="gentle" pct={62} enterFrame={NUDGE_ENTER} exitFrame={NUDGE_EXIT} />
        </div>
      )}
      ...
      {frame >= COMMAND_ENTER && (
        ...
        >
          <CommandChip command='ctx_reduce(drop="3,6")' enterFrame={COMMAND_ENTER} exitFrame={DROP_END + 30} />
        </div>
      )}
