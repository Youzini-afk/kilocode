...
export const Scene0Hook: React.FC = () => {
  ...
  const promiseProgress = interpolate(frame, [PROMISE_START, PROMISE_END], [0, 1], {
  ...
  const contextPct = interpolate(frame, [FAST_FORWARD_START, FAST_FORWARD_END], [20, 95], {
  ...
  const forgottenProgress = interpolate(frame, [FORGOTTEN_START, FORGOTTEN_END], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })
  ...
  // Pulse effect at 95%
  const pulse = frame >= PULSE_START && frame < PULSE_END ? Math.sin((frame - PULSE_START) * 0.5) * 0.3 + 0.7 : 1

  // Title card
  const titleOpacity = interpolate(frame, [TITLE_START, TITLE_START + 15, TITLE_END - 15, TITLE_END], [0, 1, 1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })
          ...
          >
            <div
              style={{
                ...
                borderRadius: 12,
                padding: "16px 20px",
                opacity: interpolate(frame, [PROMISE_START, PROMISE_START + 10], [0, 1], {
                  extrapolateLeft: "clamp",
                  extrapolateRight: "clamp",
                }),
              }}
            >
              ...
              >
                {visiblePromiseText}
                {frame < PROMISE_END && <span style={{ opacity: 0.7 }}>|</span>}
              </span>
            </div>
          </div>
        )}
          ...
          >
            <div
              style={{
                ...
                borderRadius: 12,
                padding: "16px 20px",
                opacity: interpolate(frame, [FORGOTTEN_START, FORGOTTEN_START + 10], [0, 1], {
                  extrapolateLeft: "clamp",
                  extrapolateRight: "clamp",
                }),
              }}
            >
              ...
              >
                ...
                {frame < FORGOTTEN_END && <span style={{ opacity: 0.7 }}>|</span>}
              </span>
            </div>
          </div>
        )}
