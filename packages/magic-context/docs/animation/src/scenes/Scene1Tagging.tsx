...
export const Scene1Tagging: React.FC = () => {
  ...
  const numVisible = SKELETON_MESSAGES.filter((_, i) => frame >= i * MESSAGE_STAGGER).length

  // Context bar climbs 0% → 45%
  const contextPct = interpolate(numVisible, [0, SKELETON_MESSAGES.length], [0, 45], {
  ...
  return (
    <AbsoluteFill style={{ background: COLORS.bg }}>
      ...
      >
        <ContextBar pct={contextPct} showLabel={frame > 80} labelText="Cache-aware · mutations queue until free" />
      </div>

      {/* Scene caption (Title Card) */}
      <SceneCaption text="System tags all messages." frame={globalFrame} />
    </AbsoluteFill>
  )
}
