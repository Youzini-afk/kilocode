...
export const Scene3Historian: React.FC = () => {
  ...
  // UI fade in
  const uiOpacity = interpolate(frame, [0, 15], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })
  ...
  const historianExitP = interpolate(frame, [HISTORIAN_EXIT - 30, HISTORIAN_EXIT], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })

  // Chunk ghost fly progress
  const chunkFlyP = interpolate(frame, [CHUNK_FLY_START, CHUNK_FLY_END], [0, 1], {
  ...
  const oldOpacity = interpolate(frame, [OLD_FADE_START, OLD_FADE_END], [1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })

  // Compartment enter progress
  const compartmentEnterP = interpolate(frame, [COMPARTMENT_ENTER, COMPARTMENT_ENTER + 40], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })

  // Context bar: 58% → 24%
  const contextPct = interpolate(frame, [OLD_FADE_START, OLD_FADE_END + 20], [58, 24], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  })
  ...
  // Bracket opacity
  const bracketOpacity = interpolate(frame, [10, 30], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })
    ...
    <AbsoluteFill style={{ background: COLORS.bg }}>
      ...
      >
        ...
        <div style={{ padding: "18px 20px", overflow: "hidden" }}>
          ...
          {frame >= COMPARTMENT_ENTER && <CompartmentCard enterProgress={compartmentEnterP} />}
      ...
      {frame >= HISTORIAN_ENTER && frame < HISTORIAN_EXIT && (
        <div style={{ position: "absolute", left: historianLeft, top: historianTop, opacity: uiOpacity }}>
          <AgentPanel type="historian" enterProgress={historianEnterP} exitProgress={historianExitP}>
