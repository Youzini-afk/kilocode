...
function openAICompatibleEfforts(model: Provider.Model): string[] {
  ...
  if (id.includes("gpt-5") || api.includes("gpt-5") || id.includes("codex") || api.includes("codex")) {
    efforts.unshift("minimal")
    efforts.push("xhigh", "max")
  }

  if (model.release_date >= OPENAI_NONE_EFFORT_RELEASE_DATE) efforts.unshift("none")
  if (model.release_date >= OPENAI_XHIGH_EFFORT_RELEASE_DATE) efforts.push("xhigh")
  if (id.includes("deepseek-v4") || api.includes("deepseek-v4")) efforts.push("max")
